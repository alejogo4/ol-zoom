...
async function sendEmail(email, name, lastname, zoomId) {
  var myHeaders = {
    "Authorization": "bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOm51bGwsImlzcyI6ImxqRDBUYmV3U2gycFRWV3JOOHlkT3ciLCJleHAiOjE2NzIwNzg3ODMsImlhdCI6MTY3MTQ3Mzk4M30.l0uFWYB3W19WjimX2bBe15aoBUH6F0ehqe7lu3uPFaE",
...
router.post("/upload", type, function (req, res) {
  ...
  parse(fileData, { columns: false, trim: true }, async function (err, rows) {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if(response?.code){
        result.push({status: false, message: response?.message, email: val[0]});
      }else{
